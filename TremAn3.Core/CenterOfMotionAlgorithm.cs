    ...
    public class CenterOfMotionAlgorithm
    {
        ...
        {//creating by model (saved result)
         ...
         //this whole thing is recreated affter pressin Computation button
            ...
            foreach (var res in rrm.DataResultsModels)  
            {
                if (res.DataSeriesType == DataSeriesType.ComX)
                    Results.SetComXAndFrameTimes(res.Y, res.X);
                else if (res.DataSeriesType == DataSeriesType.ComX)
                    Results.SetComY(res.Y);
                ...
                Results.DataResultsDict.Add(res.DataSeriesType, dr);
            }
            this.frameRate = frameRate;

            Rect = new SelectionRectangle(rrm.RoiModel);
        ...
        public void GetComFromCurrentARGBFrames()
        {
            ...
            Results.ComX.Add(comX);
            Results.ComY.Add(comY);
        }
        ...
        /// </summary>
        public async Task ComputeAllData()
        {
            if (!isComputed)
                ...
                await Task.Run(() =>
                {
                    ...
                    var comXNoAvg = Results.ComX.RemoveAverage();
                    var comYNoAvg = Results.ComY.RemoveAverage();

                    var frameTimes = Results.FrameTimes.Select(x => x.TotalSeconds).ToList();
                    //comx
                    ...
                    Results.DataResultsDict.Add(DataSeriesType.FreqProgress, GetFftDuringSignal(segmentSize, step));
        ...
        private DataResult GetFftDuringSignal(int segmentSize, int step)
        {
            List<double> fftProgressSignal = null;

            try
            {
                fftProgressSignal = FreqAnalysis.ComputeFftDuringSignalForTwoSignals(frameRate, Results.DataResultsDict[DataSeriesType.ComX].Y, Results.DataResultsDict[DataSeriesType.ComY].Y, segmentSize, step);
            }
            catch (WindowLongerThanSignalException ex)
            {
                return new DataResult { ErrorMessage = ex.Message };
            }
